...
static int g_model_full_attention_interval = 4;
static int g_model_hidden_size = HIDDEN_DIM;
static int g_model_moe_intermediate_size = INTERMEDIATE_DIM;
static int g_model_group_size = GROUP_SIZE;
...
static MainExpertLayout g_expert_layout = {
    .gate_w_offset = GATE_W_OFFSET,
    .gate_w_size = GATE_W_SIZE,
    .gate_s_offset = GATE_S_OFFSET,
    .gate_s_size = GATE_S_SIZE,
    .gate_b_offset = GATE_B_OFFSET,
    .gate_b_size = GATE_B_SIZE,
    .up_w_offset = UP_W_OFFSET,
    .up_w_size = UP_W_SIZE,
    .up_s_offset = UP_S_OFFSET,
    .up_s_size = UP_S_SIZE,
    .up_b_offset = UP_B_OFFSET,
    .up_b_size = UP_B_SIZE,
    .down_w_offset = DOWN_W_OFFSET,
    .down_w_size = DOWN_W_SIZE,
    .down_s_offset = DOWN_S_OFFSET,
    .down_s_size = DOWN_S_SIZE,
    .down_b_offset = DOWN_B_OFFSET,
    .down_b_size = DOWN_B_SIZE,
...
static MainExpertLayout compute_expert_layout(int hidden_size, int intermediate_size, int group_size) {
    MainExpertLayout layout = {0};
    size_t hidden = (size_t)hidden_size;
    size_t intermediate = (size_t)intermediate_size;
    size_t groups_hidden = hidden / (size_t)group_size;
    size_t groups_intermediate = intermediate / (size_t)group_size;
    size_t offset = 0;

    layout.gate_w_offset = offset;
    layout.gate_w_size = hidden * intermediate / 2;
    offset += layout.gate_w_size;
    layout.gate_s_offset = offset;
    layout.gate_s_size = intermediate * groups_hidden * sizeof(uint16_t);
    offset += layout.gate_s_size;
    layout.gate_b_offset = offset;
    layout.gate_b_size = intermediate * groups_hidden * sizeof(uint16_t);
    offset += layout.gate_b_size;

    layout.up_w_offset = offset;
    layout.up_w_size = hidden * intermediate / 2;
    offset += layout.up_w_size;
    layout.up_s_offset = offset;
    layout.up_s_size = intermediate * groups_hidden * sizeof(uint16_t);
    offset += layout.up_s_size;
    layout.up_b_offset = offset;
    layout.up_b_size = intermediate * groups_hidden * sizeof(uint16_t);
    offset += layout.up_b_size;

    layout.down_w_offset = offset;
    layout.down_w_size = hidden * intermediate / 2;
    offset += layout.down_w_size;
    layout.down_s_offset = offset;
    layout.down_s_size = hidden * groups_intermediate * sizeof(uint16_t);
    offset += layout.down_s_size;
    layout.down_b_offset = offset;
    layout.down_b_size = hidden * groups_intermediate * sizeof(uint16_t);
...
static inline size_t active_expert_size(void) {
    return g_expert_layout.expert_size;
}

static inline uint32_t active_hidden_size_u32(void) {
    return (uint32_t)g_model_hidden_size;
}

static inline uint32_t active_intermediate_size_u32(void) {
    return (uint32_t)g_model_moe_intermediate_size;
}

static inline uint32_t active_group_size_u32(void) {
    return (uint32_t)g_model_group_size;
}

static inline size_t active_hidden_bytes(void) {
    return (size_t)g_model_hidden_size * sizeof(float);
}

static inline size_t active_intermediate_bytes(void) {
    return (size_t)g_model_moe_intermediate_size * sizeof(float);
}

static MainModelConfig load_main_model_config(const char *model_path) {
    MainModelConfig cfg = {
    ...
    @autoreleasepool {
        NSArray<NSString *> *paths = @[
            [NSString stringWithFormat:@"%s/model_weights.json", model_path],
        ...
        NSString *used_path = nil;
        for (NSString *p in paths) {
            NSData *data = [NSData dataWithContentsOfFile:p options:0 error:&error];
            if (!data) continue;
        ...
        if (!root) {
                 printf("[config] No manifest found for main.m; using defaults (hidden=%d layers=%d experts=%d inter=%d group=%d)\n",
                     cfg.hidden_size, cfg.num_hidden_layers, cfg.num_experts,
                     cfg.moe_intermediate_size, cfg.group_size);
            return cfg;
        }

        NSDictionary *mcfg = root[@"config"];
        if ([mcfg isKindOfClass:[NSDictionary class]]) {
                 NSNumber *hidden = mcfg[@"hidden_size"];
            NSNumber *layers = mcfg[@"num_hidden_layers"];
            NSNumber *experts = mcfg[@"num_experts"];
                 NSNumber *intermediate = mcfg[@"moe_intermediate_size"];
                 NSNumber *group = mcfg[@"group_size"];
                 NSNumber *full_interval = mcfg[@"full_attention_interval"];
                 if ([hidden isKindOfClass:[NSNumber class]]) cfg.hidden_size = [hidden intValue];
            if ([layers isKindOfClass:[NSNumber class]]) cfg.num_hidden_layers = [layers intValue];
            if ([experts isKindOfClass:[NSNumber class]]) cfg.num_experts = [experts intValue];
                 if ([intermediate isKindOfClass:[NSNumber class]]) cfg.moe_intermediate_size = [intermediate intValue];
                 if ([group isKindOfClass:[NSNumber class]]) cfg.group_size = [group intValue];
                 if ([full_interval isKindOfClass:[NSNumber class]]) cfg.full_attention_interval = [full_interval intValue];
        }

             if (cfg.hidden_size <= 0) cfg.hidden_size = HIDDEN_DIM;
        if (cfg.num_hidden_layers <= 0) cfg.num_hidden_layers = NUM_LAYERS;
        if (cfg.num_experts <= 0) cfg.num_experts = NUM_EXPERTS;
             if (cfg.moe_intermediate_size <= 0) cfg.moe_intermediate_size = INTERMEDIATE_DIM;
             if (cfg.group_size <= 0) cfg.group_size = GROUP_SIZE;
             if (cfg.full_attention_interval <= 0) cfg.full_attention_interval = 4;

             printf("[config] Loaded manifest: %s (hidden=%d layers=%d experts=%d inter=%d group=%d full_attn_interval=%d)\n",
               used_path ? [used_path UTF8String] : "(unknown)",
                 cfg.hidden_size, cfg.num_hidden_layers, cfg.num_experts,
                 cfg.moe_intermediate_size, cfg.group_size, cfg.full_attention_interval);
    }
...
static ExpertTiming run_expert_forward(
    ...
    double t0 = now_ms();
    const MainExpertLayout *layout = &g_expert_layout;

    // ---- I/O: pread all 9 components for this expert ----
    off_t expert_offset = (off_t)expert_idx * (off_t)layout->expert_size;

    double t_io_start = now_ms();

    id<MTLBuffer> gate_w = metal_buf_pread(ctx, packed_fd, layout->gate_w_size, expert_offset + (off_t)layout->gate_w_offset);
    id<MTLBuffer> gate_s = metal_buf_pread(ctx, packed_fd, layout->gate_s_size, expert_offset + (off_t)layout->gate_s_offset);
    id<MTLBuffer> gate_b = metal_buf_pread(ctx, packed_fd, layout->gate_b_size, expert_offset + (off_t)layout->gate_b_offset);
    id<MTLBuffer> up_w   = metal_buf_pread(ctx, packed_fd, layout->up_w_size,   expert_offset + (off_t)layout->up_w_offset);
    id<MTLBuffer> up_s   = metal_buf_pread(ctx, packed_fd, layout->up_s_size,   expert_offset + (off_t)layout->up_s_offset);
    id<MTLBuffer> up_b   = metal_buf_pread(ctx, packed_fd, layout->up_b_size,   expert_offset + (off_t)layout->up_b_offset);
    id<MTLBuffer> down_w = metal_buf_pread(ctx, packed_fd, layout->down_w_size, expert_offset + (off_t)layout->down_w_offset);
    id<MTLBuffer> down_s = metal_buf_pread(ctx, packed_fd, layout->down_s_size, expert_offset + (off_t)layout->down_s_offset);
    id<MTLBuffer> down_b = metal_buf_pread(ctx, packed_fd, layout->down_b_size, expert_offset + (off_t)layout->down_b_offset);

    double t_io_end = now_ms();
    timing.io_ms = t_io_end - t_io_start;
    timing.io_bytes = layout->expert_size;
    ...
    id<MTLBuffer> gate_out = metal_buf_shared(ctx, active_intermediate_bytes());
    id<MTLBuffer> up_out   = metal_buf_shared(ctx, active_intermediate_bytes());
    id<MTLBuffer> act_out  = metal_buf_shared(ctx, active_intermediate_bytes());

    uint32_t hidden = active_hidden_size_u32();
    uint32_t inter  = active_intermediate_size_u32();
    uint32_t gs     = active_group_size_u32();
...
static ExpertTiming run_expert_forward_fast(
    ...
    double t_io_start = now_ms();
    ssize_t nread = pread(packed_fd, [expert_buf contents], layout->expert_size, expert_offset);
    double t_io_end = now_ms();

    timing.io_ms = t_io_end - t_io_start;
    timing.io_bytes = layout->expert_size;

    if (nread != (ssize_t)layout->expert_size) {
        fprintf(stderr, "ERROR: pread expert %d: got %zd, expected %zu\n",
                expert_idx, nread, layout->expert_size);
        timing.total_ms = now_ms() - t0;
        return timing;
    }
    ...
    metal_dequant_matvec_offset(ctx, cmdbuf,
        expert_buf, layout->gate_w_offset,
        expert_buf, layout->gate_s_offset,
        expert_buf, layout->gate_b_offset,
        x_buf, 0,
        gate_out, 0,
        inter, hidden, gs, use_fast);
    ...
    // up_proj: [4096] -> [1024]
    metal_dequant_matvec_offset(ctx, cmdbuf,
        expert_buf, layout->up_w_offset,
        expert_buf, layout->up_s_offset,
        expert_buf, layout->up_b_offset,
        x_buf, 0,
        up_out, 0,
        inter, hidden, gs, use_fast);
    ...
    // down_proj: [1024] -> [4096]
    metal_dequant_matvec_offset(ctx, cmdbuf,
        expert_buf, layout->down_w_offset,
        expert_buf, layout->down_s_offset,
        expert_buf, layout->down_b_offset,
        act_out, 0,
        out_buf, 0,
        hidden, inter, gs, use_fast);
...
static MoETiming run_moe_forward_fused(
    ...
    for (int k = 0; k < K_use; k++) {
        expert_bufs[k] = metal_buf_shared(ctx, layout->expert_size);
        gate_outs[k]   = metal_buf_shared(ctx, active_intermediate_bytes());
        up_outs[k]     = metal_buf_shared(ctx, active_intermediate_bytes());
        act_outs[k]    = metal_buf_shared(ctx, active_intermediate_bytes());
        expert_outs[k] = metal_buf_shared(ctx, active_hidden_bytes());
    }
    ...
    // Phase 1: gate_proj + up_proj for ALL experts (independent, GPU can overlap)
    {
        ...
        for (int k = 0; k < K_use; k++) {
            encode_matvec_v3(ctx, enc,
                expert_bufs[k], layout->gate_w_offset,
                expert_bufs[k], layout->gate_s_offset,
                expert_bufs[k], layout->gate_b_offset,
                x_buf, 0, gate_outs[k], 0,
                inter, hidden, gs);
            encode_matvec_v3(ctx, enc,
                expert_bufs[k], layout->up_w_offset,
                expert_bufs[k], layout->up_s_offset,
                expert_bufs[k], layout->up_b_offset,
                x_buf, 0, up_outs[k], 0,
                inter, hidden, gs);
        }
        [enc endEncoding];
    ...
    // Phase 3: down_proj for ALL experts (depends on phase 2)
    {
        ...
        for (int k = 0; k < K_use; k++) {
            encode_matvec_v3(ctx, enc,
                expert_bufs[k], layout->down_w_offset,
                expert_bufs[k], layout->down_s_offset,
                expert_bufs[k], layout->down_b_offset,
    ...
    {
        id<MTLBuffer> stacked = metal_buf_shared(ctx, (size_t)K_use * active_hidden_bytes());
        id<MTLBlitCommandEncoder> blit = [cmdbuf blitCommandEncoder];
        for (int k = 0; k < K_use; k++) {
            [blit copyFromBuffer:expert_outs[k]
                    sourceOffset:0
                        toBuffer:stacked
               destinationOffset:(size_t)k * active_hidden_bytes()
                            size:active_hidden_bytes()];
        }
        [blit endEncoding];
        ...
        uint32_t dim_val = active_hidden_size_u32();
        ...
        uint32_t num_tgs = (active_hidden_size_u32() + 255) / 256;
        [enc dispatchThreadgroups:MTLSizeMake(num_tgs, 1, 1)
            threadsPerThreadgroup:MTLSizeMake(256, 1, 1)];
        [enc endEncoding];
    }
...
static MoETiming run_moe_forward(
    ...
    id<MTLBuffer> expert_buf = metal_buf_shared(ctx, layout->expert_size);
    id<MTLBuffer> gate_out = metal_buf_shared(ctx, active_intermediate_bytes());
    id<MTLBuffer> up_out   = metal_buf_shared(ctx, active_intermediate_bytes());
    id<MTLBuffer> act_out  = metal_buf_shared(ctx, active_intermediate_bytes());

    // Stacked outputs for weighted combination
    id<MTLBuffer> stacked = metal_buf_shared(ctx, (size_t)K * active_hidden_bytes());
    id<MTLBuffer> expert_out = metal_buf_shared(ctx, active_hidden_bytes());
    ...
    for (int k = 0; k < K; k++) {
        ...
        // Copy this expert's output into the stacked buffer
         memcpy((float *)[stacked contents] + (size_t)k * g_model_hidden_size,
               [expert_out contents],
             active_hidden_bytes());
    }
    ...
    MTLSize numGroups = MTLSizeMake((active_hidden_size_u32() + tg_size - 1) / tg_size, 1, 1);
    [enc dispatchThreadgroups:numGroups threadsPerThreadgroup:MTLSizeMake(tg_size, 1, 1)];
    [enc endEncoding];
...
typedef struct {
    ...
    size_t size;         // active expert size
...
static void build_io_plan(ExpertIOPlan *plan, int layer_fd,
    ...
    for (int k = 0; k < K; k++) {
        ...
        plan->offset[k] = (off_t)expert_indices[k] * (off_t)active_expert_size();
    }
}
...
static double execute_io_plan(ExpertIOPlan *plan) {
    ...
    for (int k = 0; k < plan->K; k++) {
        ...
        tasks[k].size = active_expert_size();
        tasks[k].result = 0;
    }
    ...
    for (int k = 0; k < plan->K; k++) {
        if (tasks[k].result != (ssize_t)active_expert_size()) {
            fprintf(stderr, "ERROR: pread (offset=%lld): got %zd, expected %zu (errno=%d)\n",
                    (long long)tasks[k].offset, tasks[k].result, active_expert_size(), errno);
        }
    }
    return io_ms;
}
...
static FullForwardTiming run_full_forward(
    ...
    for (int k = 0; k < K; k++) {
        expert_bufs_A[k] = metal_buf_shared(ctx, layout->expert_size);
        expert_bufs_B[k] = metal_buf_shared(ctx, layout->expert_size);
    ...
    for (int k = 0; k < K; k++) {
        per_k_gate[k] = metal_buf_shared(ctx, active_intermediate_bytes());
        per_k_up[k]   = metal_buf_shared(ctx, active_intermediate_bytes());
        per_k_act[k]  = metal_buf_shared(ctx, active_intermediate_bytes());
        per_k_out[k]  = metal_buf_shared(ctx, active_hidden_bytes());
    }
    ...
    id<MTLBuffer> h_buf = metal_buf_shared(ctx, active_hidden_bytes());
    float *h_data = (float *)[h_buf contents];
    for (int i = 0; i < g_model_hidden_size; i++) {
        h_data[i] = 0.1f * sinf((float)i * 0.1f + 0.3f);
    }

    // MoE output buffer per layer
    id<MTLBuffer> moe_out = metal_buf_shared(ctx, active_hidden_bytes());
    ...
    // Uses a simple hash to spread across the configured expert set.
    int *layer_experts = calloc((size_t)g_model_num_layers * MAX_ACTIVE_EXPERTS, sizeof(int));
    if (!layer_experts) {
        fprintf(stderr, "ERROR: Failed to allocate layer_experts\n");
        timing.total_ms = now_ms() - t0;
        return timing;
    }
    for (int layer = 0; layer < g_model_num_layers; layer++) {
        for (int k = 0; k < K; k++) {
            ...
            layer_experts[layer * MAX_ACTIVE_EXPERTS + k] =
                ((layer * 7 + k * 31 + 13) % g_model_num_experts);
        }
    }
    ...
    for (int layer = 0; layer < g_model_num_layers; layer++) {
        ...
        if (layer + 1 < g_model_num_layers) {
            prefetch_start_with_plan(&prefetch, layer_fds[layer + 1],
                                      &layer_experts[(layer + 1) * MAX_ACTIVE_EXPERTS], next_bufs, K);
        }
        ...
        // Weighted sum on CPU (4 × 4096 floats = trivial, avoids extra cmd buffer)
        {
            ...
            memset(moe, 0, active_hidden_bytes());
            for (int k = 0; k < K; k++) {
                float *ek = (float *)[per_k_out[k] contents];
                float wk = w[k];
                for (int d = 0; d < g_model_hidden_size; d++) {
                    moe[d] += ek[d] * wk;
                }
            }
        }
    ...
    timing.io_bytes = (size_t)g_model_num_layers * K * layout->expert_size;
...
static void cpu_expert_forward(
    ...
    const MainExpertLayout *layout = &g_expert_layout;
    off_t expert_offset = (off_t)expert_idx * (off_t)layout->expert_size;
    uint32_t *gate_w = malloc(layout->gate_w_size);
    uint16_t *gate_s = malloc(layout->gate_s_size);
    uint16_t *gate_b = malloc(layout->gate_b_size);
    uint32_t *up_w = malloc(layout->up_w_size);
    uint16_t *up_s = malloc(layout->up_s_size);
    uint16_t *up_b = malloc(layout->up_b_size);
    uint32_t *down_w = malloc(layout->down_w_size);
    uint16_t *down_s = malloc(layout->down_s_size);
    uint16_t *down_b = malloc(layout->down_b_size);
    float *gate_out;
    float *up_out;
    float *act_out;

    if (!gate_w || !gate_s || !gate_b || !up_w || !up_s || !up_b || !down_w || !down_s || !down_b) {
        fprintf(stderr, "ERROR: failed to allocate CPU expert tensors\n");
        free(gate_w); free(gate_s); free(gate_b);
        free(up_w); free(up_s); free(up_b);
        free(down_w); free(down_s); free(down_b);
        return;
    }

    pread(packed_fd, gate_w, layout->gate_w_size, expert_offset + (off_t)layout->gate_w_offset);
    pread(packed_fd, gate_s, layout->gate_s_size, expert_offset + (off_t)layout->gate_s_offset);
    pread(packed_fd, gate_b, layout->gate_b_size, expert_offset + (off_t)layout->gate_b_offset);
    pread(packed_fd, up_w,   layout->up_w_size,   expert_offset + (off_t)layout->up_w_offset);
    pread(packed_fd, up_s,   layout->up_s_size,   expert_offset + (off_t)layout->up_s_offset);
    pread(packed_fd, up_b,   layout->up_b_size,   expert_offset + (off_t)layout->up_b_offset);
    pread(packed_fd, down_w, layout->down_w_size, expert_offset + (off_t)layout->down_w_offset);
    pread(packed_fd, down_s, layout->down_s_size, expert_offset + (off_t)layout->down_s_offset);
    pread(packed_fd, down_b, layout->down_b_size, expert_offset + (off_t)layout->down_b_offset);

    gate_out = malloc(active_intermediate_bytes());
    up_out = malloc(active_intermediate_bytes());
    act_out = malloc(active_intermediate_bytes());
    if (!gate_out || !up_out || !act_out) {
        fprintf(stderr, "ERROR: failed to allocate CPU expert activations\n");
        free(gate_w); free(gate_s); free(gate_b);
        free(up_w); free(up_s); free(up_b);
        free(down_w); free(down_s); free(down_b);
        free(gate_out); free(up_out); free(act_out);
        return;
    }

    cpu_dequant_matvec_4bit(gate_w, gate_s, gate_b, x, gate_out,
                            active_intermediate_size_u32(), active_hidden_size_u32(), active_group_size_u32());
    ...
    // SwiGLU
    cpu_swiglu(gate_out, up_out, act_out, active_intermediate_size_u32());

    // down_proj: [1024] -> [4096]
    cpu_dequant_matvec_4bit(down_w, down_s, down_b, act_out, out,
                            active_hidden_size_u32(), active_intermediate_size_u32(), active_group_size_u32());
...
int main(int argc, char **argv) {
    @autoreleasepool {
        ...
        MainModelConfig cfg = load_main_model_config(model_path);
        g_model_hidden_size = cfg.hidden_size;
        g_model_num_layers = cfg.num_hidden_layers;
        g_model_num_experts = cfg.num_experts;
        g_model_moe_intermediate_size = cfg.moe_intermediate_size;
        g_model_group_size = cfg.group_size;
        g_model_full_attention_interval = cfg.full_attention_interval;
        g_expert_layout = compute_expert_layout(cfg.hidden_size, cfg.moe_intermediate_size, cfg.group_size);

        if ((g_model_hidden_size % g_model_group_size) != 0 ||
            (g_model_moe_intermediate_size % g_model_group_size) != 0 ||
            (g_model_hidden_size % 8) != 0 ||
            (g_model_moe_intermediate_size % 8) != 0) {
            fprintf(stderr,
                "ERROR: unsupported runtime geometry hidden=%d intermediate=%d group=%d; dimensions must be divisible by group size and 8\n",
                g_model_hidden_size, g_model_moe_intermediate_size, g_model_group_size);
            return 1;
        }

        if (layer_idx < 0) layer_idx = 0;
        if (layer_idx >= g_model_num_layers) {
            fprintf(stderr, "WARNING: --layer %d out of range for %d layers, clamping\n",
                    layer_idx, g_model_num_layers);
            layer_idx = g_model_num_layers - 1;
        }
        if (expert_idx < 0) expert_idx = 0;
        if (expert_idx >= g_model_num_experts) {
            fprintf(stderr, "WARNING: --expert %d out of range for %d experts, clamping\n",
                    expert_idx, g_model_num_experts);
            expert_idx = g_model_num_experts - 1;
        }
        ...
        if (num_active_experts > g_model_num_experts) num_active_experts = g_model_num_experts;
        ...
        if (do_full) {
             printf("Mode: FULL %d-layer forward, K=%d, Shader: %s, Benchmark: %s\n",
                 g_model_num_layers, num_active_experts, shader_name,
                   do_benchmark ? "YES" : "NO");
        } else {
        ...
        if (do_full) {
            ...
            int *layer_fds = calloc(g_model_num_layers, sizeof(int));
            if (!layer_fds) {
                fprintf(stderr, "ERROR: Failed to allocate layer_fds\n");
                metal_destroy(ctx);
                return 1;
            }
            int open_count = 0;
            ...
            // ---- Run full forward pass ----
              ...
              size_t total_expert_bytes = (size_t)g_model_num_layers * K * active_expert_size();
              printf("\n=== Full %d-layer MoE forward (K=%d) ===\n", g_model_num_layers, K);
            printf("[config] %d layers x %d experts x %.2f MB = %.1f MB total I/O\n",
                  g_model_num_layers, K, active_expert_size() / (1024.0 * 1024.0),
            ...
            // Cleanup all fds
            for (int i = 0; i < g_model_num_layers; i++) close(layer_fds[i]);
            free(layer_fds);
            metal_destroy(ctx);
            printf("\nDone.\n");
            return 0;
        ...
        id<MTLBuffer> x_buf = metal_buf_shared(ctx, active_hidden_bytes());
        ...
        // ---- Output buffer ----
        id<MTLBuffer> out_buf = metal_buf_shared(ctx, active_hidden_bytes());
        ...
        if (!do_moe) {
            ...
            if (do_verify) {
                printf("\n--- CPU verification ---\n");
                float *cpu_out = calloc((size_t)g_model_hidden_size, sizeof(float));
                double t_cpu = now_ms();
            ...
            if (do_benchmark) {
                ...
                printf("[bench] I/O throughput: %.1f GB/s\n",
                       active_expert_size() * N / (io_sum * 1e6));
            }
        }
        ...
        if (do_moe) {
            ...
            float wsum = 0.0f;
            for (int k = 0; k < K; k++) {
                moe_experts[k] = (k * (g_model_num_experts / K)) % g_model_num_experts;
            ...
            printf("[timing] I/O: %.2f ms (%.1f GB/s)\n",
                   mt.io_ms, (double)(active_expert_size() * K) / (mt.io_ms * 1e6));
