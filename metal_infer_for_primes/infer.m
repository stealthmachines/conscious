...
static ModelConfig g_model_cfg = {
...
static int g_model_num_layers = NUM_LAYERS;
static int g_model_num_experts = NUM_EXPERTS;
static int g_model_num_full_attn_layers = 15;
static int g_model_num_linear_layers = 45;
static int g_use_2bit = 0;       // enabled by --2bit flag: use packed_experts_2bit/ + 2-bit kernel
static ExpertLayout g_layout_4bit = {
    .gate_w_off = 0,
    .gate_s_off = 2097152,
    .gate_b_off = 2228224,
    .up_w_off = 2359296,
...
static ExpertLayout g_layout_2bit = {
    .gate_w_off = GATE_W_OFF_2,
    .gate_s_off = GATE_S_OFF_2,
    .gate_b_off = GATE_B_OFF_2,
    .up_w_off = UP_W_OFF_2,
    .up_s_off = UP_S_OFF_2,
    .up_b_off = UP_B_OFF_2,
    .down_w_off = DOWN_W_OFF_2,
    .down_s_off = DOWN_S_OFF_2,
    .down_b_off = DOWN_B_OFF_2,
    .expert_size = EXPERT_SIZE_2BIT,
};

static ExpertLayout compute_expert_layout(int bits, int hidden, int inter, int group) {
    ExpertLayout l = {0};
    int vals_per_u32 = (bits == 2) ? 16 : 8;

    size_t gate_w = (size_t)inter * (size_t)(hidden / vals_per_u32) * sizeof(uint32_t);
    size_t gate_s = (size_t)inter * (size_t)(hidden / group) * sizeof(uint16_t);
    size_t gate_b = gate_s;

    size_t up_w = gate_w;
    size_t up_s = gate_s;
    size_t up_b = gate_b;

    size_t down_w = (size_t)hidden * (size_t)(inter / vals_per_u32) * sizeof(uint32_t);
    size_t down_s = (size_t)hidden * (size_t)(inter / group) * sizeof(uint16_t);
    size_t down_b = down_s;

    l.gate_w_off = 0;
    l.gate_s_off = l.gate_w_off + gate_w;
    l.gate_b_off = l.gate_s_off + gate_s;
    l.up_w_off = l.gate_b_off + gate_b;
    l.up_s_off = l.up_w_off + up_w;
    l.up_b_off = l.up_s_off + up_s;
    l.down_w_off = l.up_b_off + up_b;
    l.down_s_off = l.down_w_off + down_w;
    l.down_b_off = l.down_s_off + down_s;
    l.expert_size = l.down_b_off + down_b;
    return l;
}

static void refresh_expert_layouts(void) {
    g_layout_4bit = compute_expert_layout(4, g_model_cfg.hidden_size,
                                          g_model_cfg.moe_intermediate_size,
                                          g_model_cfg.group_size);
    g_layout_2bit = compute_expert_layout(2, g_model_cfg.hidden_size,
                                          g_model_cfg.moe_intermediate_size,
                                          g_model_cfg.group_size);
}

static inline const ExpertLayout *active_expert_layout(void) {
    return g_use_2bit ? &g_layout_2bit : &g_layout_4bit;
}

static inline int is_full_attn_layer(int layer_idx) {
    return ((layer_idx + 1) % g_model_cfg.full_attention_interval) == 0;
}

static inline int full_attn_layer_index(int layer_idx) {
    return (layer_idx + 1) / g_model_cfg.full_attention_interval - 1;
}

static inline int linear_attn_layer_index(int layer_idx) {
    return layer_idx - (layer_idx + 1) / g_model_cfg.full_attention_interval;
}
...
static int *g_expert_freq = NULL;  // [num_layers × num_experts] activation count, heap-allocated
...
static HDGL_History *g_hdgl_history = NULL;  // per-layer temporal routing history
...
static uint8_t *g_expert_seen = NULL;  // [num_layers × ceil(num_experts/8)] seen-bitset, heap-allocated

// Async pread state defined after InferPreadTask (see below)

static inline int expert_is_seen(int layer, int expert) {
    int stride = (g_model_num_experts + 7) / 8;
    return (g_expert_seen[(size_t)layer * stride + (expert >> 3)] >> (expert & 7)) & 1;
}
static inline void expert_mark_seen(int layer, int expert) {
    int stride = (g_model_num_experts + 7) / 8;
    g_expert_seen[(size_t)layer * stride + (expert >> 3)] |= (1 << (expert & 7));
}
...
static inline size_t active_expert_size(void) {
    return active_expert_layout()->expert_size;
}
...
static size_t g_cache_telemetry_capacity = 0;

static inline size_t cache_telemetry_index(int layer_idx, int expert_idx) {
    return (size_t)layer_idx * (size_t)g_model_num_experts + (size_t)expert_idx;
}

static int cache_telemetry_ensure_capacity(void) {
    size_t needed;
    if (!g_cache_telemetry_enabled) return 1;
    if (g_model_num_layers <= 0 || g_model_num_experts <= 0) return 0;
    needed = (size_t)g_model_num_layers * (size_t)g_model_num_experts;
    if (needed == 0) return 0;
    if (g_cache_telemetry_capacity == needed &&
        g_cache_seen && g_cache_last_touch_token && g_cache_last_evict_token) {
        return 1;
    }

    uint8_t *seen = calloc(needed, sizeof(uint8_t));
    uint64_t *touch = calloc(needed, sizeof(uint64_t));
    uint64_t *evict = calloc(needed, sizeof(uint64_t));
    if (!seen || !touch || !evict) {
        free(seen);
        free(touch);
        free(evict);
    ...
    free(g_cache_seen);
    free(g_cache_last_touch_token);
    free(g_cache_last_evict_token);
    g_cache_seen = seen;
    g_cache_last_touch_token = touch;
    g_cache_last_evict_token = evict;
    g_cache_telemetry_capacity = needed;
    return 1;
}

static void cache_telemetry_reset(void) {
    if (!cache_telemetry_ensure_capacity()) {
        fprintf(stderr, "WARNING: cache telemetry buffers unavailable for layers=%d experts=%d; disabling telemetry\n",
                g_model_num_layers, g_model_num_experts);
        g_cache_telemetry_enabled = 0;
        return;
    }
    memset(&g_cache_telemetry, 0, sizeof(g_cache_telemetry));
    memset(g_cache_seen, 0, g_cache_telemetry_capacity * sizeof(uint8_t));
    memset(g_cache_last_touch_token, 0, g_cache_telemetry_capacity * sizeof(uint64_t));
    memset(g_cache_last_evict_token, 0, g_cache_telemetry_capacity * sizeof(uint64_t));
}
...
static void cache_telemetry_touch(int layer_idx, int expert_idx) {
    size_t idx;
    if (!g_cache_telemetry_enabled) return;
    if (layer_idx < 0 || layer_idx >= g_model_num_layers ||
        expert_idx < 0 || expert_idx >= g_model_num_experts) return;
    idx = cache_telemetry_index(layer_idx, expert_idx);
    if (!g_cache_seen[idx]) {
        g_cache_seen[idx] = 1;
...
static void cache_telemetry_print(uint64_t hits, uint64_t misses) {
    ...
    fprintf(stderr, "Unique experts touched: %llu / %d (%.1f%%)\n",
            g_cache_telemetry.unique_experts_touched,
            g_model_num_layers * g_model_num_experts,
            100.0 * g_cache_telemetry.unique_experts_touched /
                (g_model_num_layers * g_model_num_experts));
...
static TensorManifest *load_manifest(const char *json_path) {
    @autoreleasepool {
        ...
        m->config.num_experts_per_tok = NUM_EXPERTS_PER_TOK;
        m->config.vocab_size = VOCAB_SIZE;
        m->config.full_attention_interval = FULL_ATTN_INTERVAL;
        m->config.moe_intermediate_size = MOE_INTERMEDIATE;
        m->config.shared_expert_intermediate_size = SHARED_INTERMEDIATE;
        m->config.group_size = GROUP_SIZE;

        NSDictionary *cfg = root[@"config"];
        if ([cfg isKindOfClass:[NSDictionary class]]) {
            NSNumber *hidden = cfg[@"hidden_size"];
            NSNumber *layers = cfg[@"num_hidden_layers"];
            NSNumber *experts = cfg[@"num_experts"];
            NSNumber *experts_per_tok = cfg[@"num_experts_per_tok"];
            NSNumber *vocab = cfg[@"vocab_size"];
            NSNumber *full_attn = cfg[@"full_attention_interval"];
            NSNumber *moe_inter = cfg[@"moe_intermediate_size"];
            NSNumber *shared_inter = cfg[@"shared_expert_intermediate_size"];
            NSNumber *group_size = cfg[@"group_size"];
            if ([hidden isKindOfClass:[NSNumber class]]) m->config.hidden_size = [hidden intValue];
            if ([layers isKindOfClass:[NSNumber class]]) m->config.num_hidden_layers = [layers intValue];
            if ([experts isKindOfClass:[NSNumber class]]) m->config.num_experts = [experts intValue];
            if ([experts_per_tok isKindOfClass:[NSNumber class]]) m->config.num_experts_per_tok = [experts_per_tok intValue];
            if ([vocab isKindOfClass:[NSNumber class]]) m->config.vocab_size = [vocab intValue];
            if ([full_attn isKindOfClass:[NSNumber class]]) m->config.full_attention_interval = [full_attn intValue];
            if ([moe_inter isKindOfClass:[NSNumber class]]) m->config.moe_intermediate_size = [moe_inter intValue];
            if ([shared_inter isKindOfClass:[NSNumber class]]) m->config.shared_expert_intermediate_size = [shared_inter intValue];
            if ([group_size isKindOfClass:[NSNumber class]]) m->config.group_size = [group_size intValue];
        }
        ...
        printf("[manifest] config: hidden=%d layers=%d experts=%d topk=%d vocab=%d full_attn_interval=%d\n",
               m->config.hidden_size, m->config.num_hidden_layers,
               m->config.num_experts, m->config.num_experts_per_tok,
               m->config.vocab_size, m->config.full_attention_interval);
         printf("[manifest] moe: intermediate=%d shared_intermediate=%d group_size=%d\n",
             m->config.moe_intermediate_size,
             m->config.shared_expert_intermediate_size,
             m->config.group_size);
        return m;
    }
}
...
static void reset_delta_net_state(void) {
    ...
    for (int i = 0; i < g_model_num_linear_layers; i++) {
...
static void gpu_encode_expert_forward_slot(
    ...
    const ExpertLayout *layout = active_expert_layout();
    NSUInteger gate_w_off = layout->gate_w_off, gate_s_off = layout->gate_s_off, gate_b_off = layout->gate_b_off;
    NSUInteger up_w_off = layout->up_w_off, up_s_off = layout->up_s_off, up_b_off = layout->up_b_off;
    NSUInteger down_w_off = layout->down_w_off, down_s_off = layout->down_s_off, down_b_off = layout->down_b_off;
...
static void gpu_encode_expert_forward(
    ...
    const ExpertLayout *layout = active_expert_layout();
    NSUInteger gate_w_off = layout->gate_w_off;
    NSUInteger gate_s_off = layout->gate_s_off;
    NSUInteger gate_b_off = layout->gate_b_off;
    NSUInteger up_w_off   = layout->up_w_off;
    NSUInteger up_s_off   = layout->up_s_off;
    NSUInteger up_b_off   = layout->up_b_off;
    NSUInteger down_w_off = layout->down_w_off;
    NSUInteger down_s_off = layout->down_s_off;
    NSUInteger down_b_off = layout->down_b_off;
...
static void moe_forward(
    ...
    float *gate_scores = calloc(g_model_num_experts, sizeof(float));
    ...
    if (gate_w && gate_s && gate_b && sgw && sgs && sgb &&
        ...
        BatchMatvecSpec moe_specs[4] = {
            { gate_w, gate_s, gate_b, gate_scores,        (uint32_t)g_model_num_experts, HIDDEN_DIM, GROUP_SIZE, 0 },
    ...
    cpu_softmax(gate_scores, g_model_num_experts);

    if (g_use_hdgl && g_hdgl_lattice && g_hdgl_history && layer_idx < g_model_num_layers) {
        char tok_buf[32];
        ...
        if (hdgl_expert >= 0 && hdgl_expert < g_model_num_experts)
            gate_scores[hdgl_expert] += g_hdgl_alpha;
        cpu_softmax(gate_scores, g_model_num_experts);
    }

    // Top-K expert selection
    int *expert_indices = malloc((size_t)K * sizeof(int));
    float *expert_weights = malloc((size_t)K * sizeof(float));
    if (!expert_indices || !expert_weights) {
        fprintf(stderr, "ERROR: MoE routing allocation failed for K=%d\n", K);
        free(expert_indices);
        free(expert_weights);
        free(h_post);
        free(h_mid);
    ...
    cpu_topk(gate_scores, g_model_num_experts, K, expert_indices, expert_weights);
...
static void reset_prediction_state(void) {
...
static ExpertLRUCache *expert_cache_new(id<MTLDevice> device, int max_entries) {
    ...
    cache->hits = 0;
    cache->misses = 0;
    for (int l = 0; l < g_model_num_layers; l++) {
        for (int e = 0; e < g_model_num_experts; e++) {
...
static id<MTLBuffer> expert_cache_lookup(ExpertLRUCache *cache, int layer_idx, int expert_idx) {
    if (layer_idx < 0 || layer_idx >= g_model_num_layers ||
        expert_idx < 0 || expert_idx >= g_model_num_experts) {
        cache->misses++;
        return nil;
    }
...
static id<MTLBuffer> malloc_cache_lookup(MallocExpertCache *cache, int layer, int expert) {
    if (layer < 0 || layer >= g_model_num_layers ||
        expert < 0 || expert >= g_model_num_experts) {
        cache->misses++;
        return nil;
    }
...
static int validate_manifest_contract(WeightFile *wf) {
    if (!wf || !wf->manifest) return 0;

    int missing = 0;
    char name[256];

    #define REQUIRE_TENSOR_STR(tname) \
        do { \
            if (!find_tensor(wf->manifest, (tname))) { \
                if (missing < 16) fprintf(stderr, "[manifest] missing required tensor: %s\\n", (tname)); \
                missing++; \
            } \
        } while (0)

    #define REQUIRE_TENSOR_FMT(fmt, idx) \
        do { \
            snprintf(name, sizeof(name), (fmt), (idx)); \
            REQUIRE_TENSOR_STR(name); \
        } while (0)

    REQUIRE_TENSOR_STR("model.embed_tokens.weight");
    REQUIRE_TENSOR_STR("model.norm.weight");
    REQUIRE_TENSOR_STR("lm_head.weight");

    for (int i = 0; i < g_model_num_layers; i++) {
        int is_full = is_full_attn_layer(i);

        REQUIRE_TENSOR_FMT("model.layers.%d.input_layernorm.weight", i);
        REQUIRE_TENSOR_FMT("model.layers.%d.post_attention_layernorm.weight", i);

        if (is_full) {
            REQUIRE_TENSOR_FMT("model.layers.%d.self_attn.q_proj.weight", i);
            REQUIRE_TENSOR_FMT("model.layers.%d.self_attn.q_proj.scales", i);
            REQUIRE_TENSOR_FMT("model.layers.%d.self_attn.q_proj.biases", i);
            REQUIRE_TENSOR_FMT("model.layers.%d.self_attn.k_proj.weight", i);
            REQUIRE_TENSOR_FMT("model.layers.%d.self_attn.k_proj.scales", i);
            REQUIRE_TENSOR_FMT("model.layers.%d.self_attn.k_proj.biases", i);
            REQUIRE_TENSOR_FMT("model.layers.%d.self_attn.v_proj.weight", i);
            REQUIRE_TENSOR_FMT("model.layers.%d.self_attn.v_proj.scales", i);
            REQUIRE_TENSOR_FMT("model.layers.%d.self_attn.v_proj.biases", i);
            REQUIRE_TENSOR_FMT("model.layers.%d.self_attn.o_proj.weight", i);
            REQUIRE_TENSOR_FMT("model.layers.%d.self_attn.o_proj.scales", i);
            REQUIRE_TENSOR_FMT("model.layers.%d.self_attn.o_proj.biases", i);
            REQUIRE_TENSOR_FMT("model.layers.%d.self_attn.q_norm.weight", i);
            REQUIRE_TENSOR_FMT("model.layers.%d.self_attn.k_norm.weight", i);
        } else {
            REQUIRE_TENSOR_FMT("model.layers.%d.linear_attn.in_proj_qkv.weight", i);
            REQUIRE_TENSOR_FMT("model.layers.%d.linear_attn.in_proj_qkv.scales", i);
            REQUIRE_TENSOR_FMT("model.layers.%d.linear_attn.in_proj_qkv.biases", i);
            REQUIRE_TENSOR_FMT("model.layers.%d.linear_attn.in_proj_z.weight", i);
            REQUIRE_TENSOR_FMT("model.layers.%d.linear_attn.in_proj_z.scales", i);
            REQUIRE_TENSOR_FMT("model.layers.%d.linear_attn.in_proj_z.biases", i);
            REQUIRE_TENSOR_FMT("model.layers.%d.linear_attn.in_proj_b.weight", i);
            REQUIRE_TENSOR_FMT("model.layers.%d.linear_attn.in_proj_b.scales", i);
            REQUIRE_TENSOR_FMT("model.layers.%d.linear_attn.in_proj_b.biases", i);
            REQUIRE_TENSOR_FMT("model.layers.%d.linear_attn.in_proj_a.weight", i);
            REQUIRE_TENSOR_FMT("model.layers.%d.linear_attn.in_proj_a.scales", i);
            REQUIRE_TENSOR_FMT("model.layers.%d.linear_attn.in_proj_a.biases", i);
            REQUIRE_TENSOR_FMT("model.layers.%d.linear_attn.conv1d.weight", i);
            REQUIRE_TENSOR_FMT("model.layers.%d.linear_attn.A_log", i);
            REQUIRE_TENSOR_FMT("model.layers.%d.linear_attn.dt_bias", i);
            REQUIRE_TENSOR_FMT("model.layers.%d.linear_attn.norm.weight", i);
            REQUIRE_TENSOR_FMT("model.layers.%d.linear_attn.out_proj.weight", i);
            REQUIRE_TENSOR_FMT("model.layers.%d.linear_attn.out_proj.scales", i);
            REQUIRE_TENSOR_FMT("model.layers.%d.linear_attn.out_proj.biases", i);
        }

        REQUIRE_TENSOR_FMT("model.layers.%d.mlp.gate.weight", i);
        REQUIRE_TENSOR_FMT("model.layers.%d.mlp.gate.scales", i);
        REQUIRE_TENSOR_FMT("model.layers.%d.mlp.gate.biases", i);
        REQUIRE_TENSOR_FMT("model.layers.%d.mlp.shared_expert.gate_proj.weight", i);
        REQUIRE_TENSOR_FMT("model.layers.%d.mlp.shared_expert.gate_proj.scales", i);
        REQUIRE_TENSOR_FMT("model.layers.%d.mlp.shared_expert.gate_proj.biases", i);
        REQUIRE_TENSOR_FMT("model.layers.%d.mlp.shared_expert.up_proj.weight", i);
        REQUIRE_TENSOR_FMT("model.layers.%d.mlp.shared_expert.up_proj.scales", i);
        REQUIRE_TENSOR_FMT("model.layers.%d.mlp.shared_expert.up_proj.biases", i);
        REQUIRE_TENSOR_FMT("model.layers.%d.mlp.shared_expert.down_proj.weight", i);
        REQUIRE_TENSOR_FMT("model.layers.%d.mlp.shared_expert.down_proj.scales", i);
        REQUIRE_TENSOR_FMT("model.layers.%d.mlp.shared_expert.down_proj.biases", i);
        REQUIRE_TENSOR_FMT("model.layers.%d.mlp.shared_expert_gate.weight", i);
        REQUIRE_TENSOR_FMT("model.layers.%d.mlp.shared_expert_gate.scales", i);
        REQUIRE_TENSOR_FMT("model.layers.%d.mlp.shared_expert_gate.biases", i);
    }

    #undef REQUIRE_TENSOR_FMT
    #undef REQUIRE_TENSOR_STR

    if (missing > 0) {
        if (missing > 16) {
            fprintf(stderr, "[manifest] ... and %d more missing tensors\\n", missing - 16);
        }
        fprintf(stderr, "ERROR: manifest contract validation failed (%d missing required tensors)\\n", missing);
        return 0;
    }

    printf("[manifest] contract validation passed for %d layers\\n", g_model_num_layers);
    return 1;
}
...
static float *s_gate_scores = NULL; // [runtime num_experts]
static float *s_spec_gate_scores = NULL; // [runtime num_experts] speculative routing scratch
...
static void init_layer_scratch(void) {
    ...
    s_gate_scores = calloc((size_t)g_model_num_experts, sizeof(float));
    s_spec_gate_scores = calloc((size_t)g_model_num_experts, sizeof(float));
...
static void fused_layer_forward(
    ...
    if (!is_full) {
        linear_layer_idx = linear_attn_layer_index(layer_idx);
    }
    ...
    int can_gpu_linear = (gpu_linear_attn_enabled &&
                          ...
                          g_metal->wf_buf &&
                          linear_layer_idx >= 0 && linear_layer_idx < g_model_num_linear_layers &&
    ...
    if (spec_routing_enabled && (g_expert_cache || g_malloc_cache) && packed_fd >= 0 && lc->gate_w) {
        ...
        memset(spec_scores, 0, g_model_num_experts * sizeof(float));
        ...
        cpu_dequant_matvec(lc->gate_w, lc->gate_s, lc->gate_b,
                   normed, spec_scores,
                   g_model_num_experts, HIDDEN_DIM, GROUP_SIZE);
        cpu_softmax(spec_scores, g_model_num_experts);

        int spec_K = (K > MAX_K) ? MAX_K : K;
        float spec_weights[MAX_K];
        cpu_topk(spec_scores, g_model_num_experts, spec_K, s_spec_indices, spec_weights);
        s_spec_count = spec_K;
    ...
    if (is_full) {
        ...
        int fa_idx = full_attn_layer_index(layer_idx);
        if (g_metal && g_metal->attn_scores_pipe && fa_idx >= 0 && fa_idx < g_model_num_full_attn_layers) {
        ...
        int gpu_attn_ready = (g_metal && g_metal->attn_scores_pipe &&
                              fa_idx >= 0 && fa_idx < g_model_num_full_attn_layers &&
        ...
        if (!linear_attn_bypass) {
            ...
            int linear_layer_idx = linear_attn_layer_index(layer_idx);
    ...
    memset(gate_scores, 0, g_model_num_experts * sizeof(float));
    ...
    float *expert_weights = malloc((size_t)K * sizeof(float));
    if (!expert_indices || !expert_weights) {
        fprintf(stderr, "ERROR: fused_layer_forward routing allocation failed for K=%d\n", K);
        free(expert_indices);
        free(expert_weights);
        return;
    }
    ...
    if (g_freq_tracking) {
        for (int k = 0; k < K; k++) {
            g_expert_freq[(size_t)layer_idx * g_model_num_experts + expert_indices[k]]++;
        }
    ...
    if (packed_fd >= 0 && g_metal && g_metal->buf_multi_expert_data[0]) {
        ...
        if (g_pred_enabled && g_pred_generating) {
            ...
            if (layer_idx == g_model_num_layers - 1) {
                g_pred_valid = 1;
            }
        }
        ...
        int gpu_combine = (g_metal->moe_combine_residual &&
                           ...
                           g_metal->wf_buf &&
                   layer_idx < g_model_num_layers - 1 &&
...
static void freq_print_analysis(int K) {
    ...
    int *sorted = (int *)malloc((size_t)g_model_num_experts * sizeof(int));
    if (!sorted) return;
    for (int l = 0; l < g_model_num_layers; l++) {
        // Count unique experts and sort frequencies descending
        memcpy(sorted, g_expert_freq + (size_t)l * g_model_num_experts, g_model_num_experts * sizeof(int));
        qsort(sorted, g_model_num_experts, sizeof(int), freq_cmp_desc);
    ...
    double avg_experts_80 = (double)experts_for_80_total / g_model_num_layers;
    // Expert size in GB: each expert is active_expert_size() bytes
    double expert_gb = (double)active_expert_size() / (1024.0 * 1024.0 * 1024.0);
    double total_pin_gb = avg_experts_80 * g_model_num_layers * expert_gb;
    ...
    uint16_t *final_norm_w, int K)
{
    ...
    if (sys_pt && sys_pt->count > 0) {
        ...
        for (int i = 0; i < sys_pt->count - 1; i++) {
            ...
            for (int layer = 0; layer < g_model_num_layers; layer++) {
                int is_full = is_full_attn_layer(layer);
            ...
            if (is_continuation) {
                ...
                for (int i = 0; i < g_model_num_layers; i++) {
                    if (kv_caches[i] && kv_snapshots[i].k_snapshot) {
                        ...
                        if (g_metal) {
                            int fa_idx = full_attn_layer_index(i);
                            if (fa_idx >= 0 && fa_idx < g_model_num_full_attn_layers) {
...
static void print_usage(const char *prog) {
    ...
    printf("  --serve PORT         Run HTTP server (OpenAI-compatible API, localhost-only)\n");
...
int main(int argc, char **argv) {
    @autoreleasepool {
        ...
        g_model_cfg = wf->manifest->config;
        g_model_num_layers = g_model_cfg.num_hidden_layers;
        g_model_num_experts = g_model_cfg.num_experts;

        if (g_model_cfg.hidden_size != HIDDEN_DIM || g_model_cfg.vocab_size != VOCAB_SIZE) {
            fprintf(stderr,
                    "WARNING: Model geometry mismatch (hidden=%d/%d vocab=%d/%d); Metal shaders require compile-time dims. Disabling GPU linear attention.\n",
                    g_model_cfg.hidden_size, HIDDEN_DIM, g_model_cfg.vocab_size, VOCAB_SIZE);
            gpu_linear_attn_enabled = 0;
        }
        if (g_model_num_layers <= 0 || g_model_num_layers > NUM_LAYERS ||
            g_model_num_experts <= 0 || g_model_num_experts > NUM_EXPERTS) {
            fprintf(stderr,
                    "ERROR: Model config out of current bounds (layers 1-%d, experts 1-%d). Got layers=%d experts=%d\n",
                    NUM_LAYERS, NUM_EXPERTS, g_model_num_layers, g_model_num_experts);
            return 1;
        }
        if (g_model_cfg.full_attention_interval <= 0) {
            fprintf(stderr, "ERROR: full_attention_interval must be > 0\n");
            return 1;
        }

        g_model_num_full_attn_layers = g_model_num_layers / g_model_cfg.full_attention_interval;
        g_model_num_linear_layers = g_model_num_layers - g_model_num_full_attn_layers;
        if (g_model_num_full_attn_layers > NUM_FULL_ATTN_LAYERS ||
            g_model_num_linear_layers > NUM_LINEAR_LAYERS) {
            fprintf(stderr,
                    "ERROR: Model requires full-attn=%d and linear=%d layers, but runtime capacity is full-attn=%d linear=%d\n",
                    g_model_num_full_attn_layers, g_model_num_linear_layers,
                    NUM_FULL_ATTN_LAYERS, NUM_LINEAR_LAYERS);
            return 1;
        }
        if (g_model_cfg.moe_intermediate_size != MOE_INTERMEDIATE ||
            g_model_cfg.shared_expert_intermediate_size != SHARED_INTERMEDIATE ||
            g_model_cfg.group_size != GROUP_SIZE) {
            fprintf(stderr,
                    "WARNING: MoE geometry mismatch (moe_intermediate=%d/%d shared=%d/%d group=%d/%d); Metal expert shaders require compile-time dims. Disabling GPU linear attention.\n",
                    g_model_cfg.moe_intermediate_size, MOE_INTERMEDIATE,
                    g_model_cfg.shared_expert_intermediate_size, SHARED_INTERMEDIATE,
                    g_model_cfg.group_size, GROUP_SIZE);
            gpu_linear_attn_enabled = 0;
        }

        refresh_expert_layouts();

        // Allocate runtime-sized expert tracking arrays (heap, flat [layer * num_experts + expert])
        free(g_expert_freq);
        g_expert_freq = (int *)calloc((size_t)g_model_num_layers * g_model_num_experts, sizeof(int));
        if (!g_expert_freq) { fprintf(stderr, "ERROR: OOM allocating expert freq table\n"); return 1; }

        free(g_expert_seen);
        g_expert_seen = (uint8_t *)calloc((size_t)g_model_num_layers * ((g_model_num_experts + 7) / 8), sizeof(uint8_t));
        if (!g_expert_seen) { fprintf(stderr, "ERROR: OOM allocating expert seen bitset\n"); return 1; }

        if (K > g_model_cfg.num_experts_per_tok) {
            printf("[config] Requested K=%d exceeds manifest num_experts_per_tok=%d, clamping\n",
                   K, g_model_cfg.num_experts_per_tok);
            K = g_model_cfg.num_experts_per_tok;
        }
        if (K > MAX_K) {
            printf("[config] Requested K=%d exceeds MAX_K=%d, clamping\n", K, MAX_K);
            K = MAX_K;
        }

        if (!validate_manifest_contract(wf)) {
            return 1;
        }
        ...
        printf("Manifest: %s\n", manifest_path);
        printf("Vocab:    %s\n", vocab_path);
        printf("Config:   hidden=%d layers=%d (full=%d linear=%d) experts=%d full_attn_interval=%d\n",
               g_model_cfg.hidden_size, g_model_num_layers,
               g_model_num_full_attn_layers, g_model_num_linear_layers,
               g_model_num_experts, g_model_cfg.full_attention_interval);
        ...
        free(g_hdgl_history);
        g_hdgl_history = calloc((size_t)g_model_num_layers, sizeof(HDGL_History));
        if (!g_hdgl_history) {
            fprintf(stderr, "ERROR: Failed to allocate HDGL history for %d layers\n", g_model_num_layers);
            return 1;
        }

        if (g_use_hdgl) {
            int hdgl_instances = g_model_cfg.hidden_size > 0 ? g_model_cfg.hidden_size : 4096;
            int hdgl_slots_per_instance = BLZ_SLOTS_PER_INST;
            printf("\n[HDGL-28] BootloaderZ APA lattice v%s\n", HDGL_VERSION_STR);
            printf("[HDGL-28] Lattice dimensions: instances=%d slots_per_instance=%d\n",
                   hdgl_instances, hdgl_slots_per_instance);
            g_hdgl_lattice = lattice_init(hdgl_instances, hdgl_slots_per_instance);
            if (!g_hdgl_lattice) {
                fprintf(stderr,
                        "ERROR: HDGL lattice_init failed (instances=%d slots_per_instance=%d)\n",
                        hdgl_instances, hdgl_slots_per_instance);
                return 1;
            }
            if (g_hdgl_load_path) {
            ...
            hdgl_router_init(g_hdgl_lattice, g_model_num_experts);
        }
        ...
        for (int i = 0; i < g_model_num_layers; i++) {
         ...
         printf("[experts] %d/%d packed layer files available (mmap'd)\n",
             expert_layers_available, g_model_num_layers);
            ...
            if (!g_use_2bit && access(lz4_probe, R_OK) == 0) {
                ...
                for (int i = 0; i < g_model_num_layers; i++) {
                    ...
                    if (lz4_fd >= 0) {
                        ...
                        g_lz4_index[i] = malloc(g_model_num_experts * sizeof(LZ4IndexEntry));
                        ssize_t nr = pread(lz4_fd, g_lz4_index[i],
                                           g_model_num_experts * sizeof(LZ4IndexEntry), 0);
                        if (nr == g_model_num_experts * (ssize_t)sizeof(LZ4IndexEntry)) {
                ...
                if (lz4_layers > 0) {
                    ...
                    for (int k = 0; k < MAX_K; k++) {
                        g_lz4_comp_bufs[k] = malloc(active_expert_size() + 4096);
                    }
                    printf("[lz4] %d/%d layers using LZ4 compressed experts\n",
                           lz4_layers, g_model_num_layers);
                }
            }
        }
        ...
        void **layer_states = calloc(g_model_num_layers, sizeof(void *));
        KVCache **kv_caches = calloc(g_model_num_layers, sizeof(KVCache *));
